...
class SalesmanController extends CI_Controller
{
 	public function __construct()
    {
 	...
 	public function index()
 	{
 		...
 		$data['page'] = 'salesman/list_salesman';
	...
	{
		$this->d_distributormodel->CSRFVerify();
		$data['page'] = 'salesman/add_salesman';
		$this->load->view('distributor/template',$data);
	}
 	...
 	public function add_salesman()
 	{
		...
		{	
			...
			$check = $this->salesmanmodel->add_salesman($params);
			
			if($check)
			{
				$this->session->set_flashdata('add_success', 'Salesman has been added Successfully..');
				redirect('distributor/salesman-list');
			}
	...
	public function edit_salesman()
	{
		...
		$data['page'] = 'salesman/edit_salesman';
		$data['user'] = $this->salesmanmodel->get_salesman_by_id($user_id); 
		$this->load->view('distributor/template',$data);
 	}
		...
		{	
			...
			if (!empty($_FILES['image']['name']))
			{
				...
				{ 
					...
					$image_name = $this->salesmanmodel->get_image($user_id);
			...
			$check = $this->salesmanmodel->update_salesman_by_id($user_id,$params);
			if($check)
			{
				$this->session->set_flashdata('add_success', 'Salesman has been updated Successfully..');
				redirect('distributor/salesman-list');
			}
		}
		$data['page'] = 'salesman/edit_salesman';
	...
	public function trash_salesman()
	{ 
		$this->d_distributormodel->CSRFVerify();
		if(!empty($_REQUEST['user_id']))
		{
			$user_id = $_REQUEST['user_id'];
			$image = $this->salesmanmodel->get_image($user_id);
			...
			$this->session->set_flashdata('del_success','Salesman has been Successfully Deleted.');
			redirect('distributor/salesman-list');
		}
	}
}