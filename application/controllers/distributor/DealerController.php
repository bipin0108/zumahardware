...
class DealerController extends CI_Controller
{
	...
	public function create_dealer()
	{
 	...
 	public function add_dealer()
 	{
		...
		{	
			...
			$check = $this->dealermodel->add_dealer($params);
			
			if($check)
			{
				$this->session->set_flashdata('add_success', 'Dealer has been added Successfully..');
				redirect('distributor/dealer-list');
			}
	...
	public function edit_dealer()
	{
		...
		$data['page'] = 'dealer/edit_dealer';
		$data['user'] = $this->dealermodel->get_dealer_by_id($user_id); 
		$this->load->view('distributor/template',$data);
 	}
		...
		{	
			...
			if (!empty($_FILES['image']['name']))
			{
				...
				{ 
					...
					$image_name = $this->dealermodel->get_image($user_id);
			...
			$check = $this->dealermodel->update_dealer_by_id($user_id,$params);
			if($check)
			{
				$this->session->set_flashdata('add_success', 'Dealer has been updated Successfully..');
				redirect('distributor/dealer-list');
			}
		}
		$data['page'] = 'dealer/edit_dealer';
	...
	public function trash_dealer()
	{ 
		$this->d_distributormodel->CSRFVerify();
		if(!empty($_REQUEST['user_id']))
		{
			$user_id = $_REQUEST['user_id'];
			$image = $this->dealermodel->get_image($user_id);
			...
			$this->session->set_flashdata('del_success','Dealer has been Successfully Deleted.');
			redirect('distributor/dealer-list');
		}
	}
	public function translation_history()
  	{
 		...
 		$data['page'] = 'dealer/transaction_history';
		$this->load->view('distributor/template',$data);
  	}
}
